...
#    my $orpha_combo = $disorders1{$key}->{OrphaNumber} . " " . $disorders1{$key}->{Name}->{content};
#    if (exists $icd10_orpha_mapping {$icd_num}) {
#      my $old_combo = $icd10_orpha_mapping {$icd_num};
#      $icd10_orpha_mapping {$icd_num} = $old_combo . "; " . $orpha_combo;
#    } else {
#      $icd10_orpha_mapping {$icd_num} = $orpha_combo;
#    }
#  }
...
#print Dumper (\%icd9_orpha_mapping);


my $input = shift;

my $icd_output = 'icd10_mapping_'.$input;
my $orpha_output = 'orpha_mapping_'.$input;
...
#      if ($orpha_name {$icd9_orpha_mapping {$element}}) {
